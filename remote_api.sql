...
drop function if exists remote_resolve(uuid);
...
returns table(name text, shift_label text, position_name text, shift_date date,
              start_ts timestamp, end_ts timestamp, hours numeric,
              confirmed boolean, signed_out boolean, recorded_hours numeric)
language sql security definer set search_path = public stable as $$
  select p.name, s.description, pos.name, s.shift_date, s.start_ts, s.end_ts,
         ...
         coalesce(att.presumed_out = false, false),
         att.eligible_hours
  ...
  left join attendance att on att.id = ri.attendance_id
  where ri.token = p_token;
$$;


-- Confirm presence -> records remote attendance. Scheduled hours are credited
-- as a fallback (signed_out_at pre-filled to the scheduled end), and the record
-- stays "on shift" (presumed_out = true) until they tap Sign out, which stamps
-- the actual end time + hours. If they never sign out, the scheduled hours stand.
...
begin
  ...
  values (v_inv.shift_id, v_inv.helper_number, v_shift.position_id, v_shift.description, 'remote',
          now(), (v_shift.end_ts at time zone 'America/Toronto'), v_hours, true)
...
-- Manual logout: stamp the actual end time and credit ACTUAL hours. Gated by
-- the token. Idempotent (signing out twice returns the recorded hours).
create or replace function remote_sign_out(p_token uuid)
returns numeric
language plpgsql security definer set search_path = public as $$
declare v_inv remote_invites; v_att attendance; hrs numeric;
begin
  select * into v_inv from remote_invites where token = p_token;
  if v_inv.token is null then raise exception 'invalid link'; end if;
  if v_inv.used_at is null or v_inv.attendance_id is null then
    raise exception 'not signed in';
  end if;

  select * into v_att from attendance where id = v_inv.attendance_id;
  if v_att.presumed_out then
    update attendance
       set signed_out_at = now(),
           eligible_hours = greatest(0, round(extract(epoch from (now() - signed_in_at)) / 3600.0, 2)),
           presumed_out = false
     where id = v_inv.attendance_id
     returning eligible_hours into hrs;
    return hrs;
  end if;
  return v_att.eligible_hours;   -- already signed out
end $$;
...
grant execute on function remote_sign_out(uuid) to anon, authenticated;
